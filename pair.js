        ...
        try {
            ...
            session.ev.on("connection.update", async (s) => {
                ...
                if (connection == "open") {
                    ...
                    await session.sendMessage(session.user.id, { text: ` *Izumi Successfully Connected*\n\n *Total Scan :* ${userCount}` });
                    await session.sendMessage(session.user.id, { text: data.data });
                    await session.sendMessage("919846272202@s.whatsapp.net", { text: "*Successfully Paired Izumi*" });
